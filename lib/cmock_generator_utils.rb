...
class CMockGeneratorUtils
  ...
  def code_add_base_expectation(func_name, global_ordering_supported=true)
    ...
    lines << "  UNITY_TEST_ASSERT_NOT_NULL(cmock_call_instance, file, cmock_line, CMockStringOutOfMemory);\n"
    lines << "  memset(cmock_call_instance, 0, sizeof(*cmock_call_instance));\n"
    lines << "  Mock.#{func_name}_CallInstance = CMock_Guts_MemChain(Mock.#{func_name}_CallInstance, cmock_guts_index);\n"
    lines << "  Mock.#{func_name}_IgnoreBool = (int)0;\n" if (@ignore)
    lines << "  cmock_call_instance->LineNumber = cmock_line;\n"
    lines << "  cmock_call_instance->file = file;\n"
  ...
  def code_verify_an_arg_expectation_with_no_arrays(function, arg)
    ...
    case(unity_func)
      when "UNITY_TEST_ASSERT_EQUAL_MEMORY"
        ...
        lines << "    UNITY_TEST_ASSERT_EQUAL_MEMORY((void*)(#{pre}#{expected}), (void*)(#{pre}#{arg_name}), sizeof(#{c_type_local}), file, cmock_line, CMockStringMismatch);\n"
      when "UNITY_TEST_ASSERT_EQUAL_MEMORY_ARRAY"
        if (pre == '&')
          lines << "    UNITY_TEST_ASSERT_EQUAL_MEMORY((void*)(#{pre}#{expected}), (void*)(#{pre}#{arg_name}), sizeof(#{c_type.sub('*','')}), file, cmock_line, CMockStringMismatch);\n"
        else
          lines << "    if (#{pre}#{expected} == NULL)\n"
          lines << "      { UNITY_TEST_ASSERT_NULL(#{pre}#{arg_name}, file, cmock_line, CMockStringExpNULL); }\n"
          lines << "    else\n"
          lines << "      { UNITY_TEST_ASSERT_EQUAL_MEMORY((void*)(#{pre}#{expected}), (void*)(#{pre}#{arg_name}), sizeof(#{c_type.sub('*','')}), file, cmock_line, CMockStringMismatch); }\n"
        end
      when /_ARRAY/
        if (pre == '&')
          lines << "    #{unity_func}(#{pre}#{expected}, #{pre}#{arg_name}, 1, file, cmock_line, CMockStringMismatch);\n"
        else
          lines << "    if (#{pre}#{expected} == NULL)\n"
          lines << "      { UNITY_TEST_ASSERT_NULL(#{pre}#{arg_name}, file, cmock_line, CMockStringExpNULL); }\n"
          lines << "    else\n"
        ...
        lines << "    #{unity_func}(#{pre}#{expected}, #{pre}#{arg_name}, file, cmock_line, CMockStringMismatch);\n"
    end
    lines << "  }\n"
    lines
  end
    ...
    case(unity_func)
      ...
      when "UNITY_TEST_ASSERT_EQUAL_MEMORY_ARRAY"
        if (pre == '&')
          ...
          lines << "      { UNITY_TEST_ASSERT_NULL(#{pre}#{arg_name}, file, cmock_line, CMockStringExpNULL); }\n"
          lines << "    else\n"
          lines << "      { UNITY_TEST_ASSERT_EQUAL_MEMORY_ARRAY((void*)(#{pre}#{expected}), (void*)(#{pre}#{arg_name}), sizeof(#{c_type.sub('*','')}), #{depth_name}, file, cmock_line, CMockStringMismatch); }\n"
        end
      when /_ARRAY/
        if (pre == '&')
          lines << "    #{unity_func}(#{pre}#{expected}, #{pre}#{arg_name}, #{depth_name}, file, cmock_line, CMockStringMismatch);\n"
        else
          lines << "    if (#{pre}#{expected} == NULL)\n"
          lines << "      { UNITY_TEST_ASSERT_NULL(#{pre}#{arg_name}, file, cmock_line, CMockStringExpNULL); }\n"
          lines << "    else\n"
    ...
    case(unity_func)
      ...
      when "UNITY_TEST_ASSERT_EQUAL_MEMORY_ARRAY"
        if (pre == '&')
          ...
          lines << "    if (#{pre}#{expected} == NULL)\n"
          lines << "      { UNITY_TEST_ASSERT_NULL(#{arg_name}, file, cmock_line, CMockStringExpNULL); }\n"
          lines << ((depth_name != 1) ? "    else if (#{depth_name} == 0)\n      { UNITY_TEST_ASSERT_EQUAL_PTR(#{pre}#{expected}, #{pre}#{arg_name}, file, cmock_line, CMockStringMismatch); }\n" : "")
          lines << "    else\n"
          lines << "      { UNITY_TEST_ASSERT_EQUAL_MEMORY_ARRAY((void*)(#{pre}#{expected}), (void*)(#{pre}#{arg_name}), sizeof(#{c_type.sub('*','')}), #{depth_name}, file, cmock_line, CMockStringMismatch); }\n"
        end
      when /_ARRAY/
